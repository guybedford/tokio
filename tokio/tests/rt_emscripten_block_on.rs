...
//! suspending. A future that would have to await a timer, an external wake,
//! or a host turn (even a lone `yield_now` defers to one) parks the stack via
//! JSPI (see `rt_emscripten_jspi`); on a runtime built with
//! `emscripten_jspi(false)` it panics instead, since without JSPI the host
//! event loop cannot run while wasm is on the stack.
//!
//! Everything here is exercisable with **no JSPI linked at all** (the CI job
//! runs this file without `-sJSPI`): the passing cases never park, and the
//! `should_panic` cases panic before parking.
