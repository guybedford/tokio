...
//!
//! `tokio::io::stdin` reads fd 0 synchronously in emscripten (the read does not
//! suspend under JSPI like other blocking syscalls — an emscripten limitation),
//! so this only completes when stdin is non-interactive (EOF), as under CI where
