//! Pins the `tokio::emscripten::event_loop` host-context contract: `schedule`
//! queues only, and wakes arriving from host context never drive inline. Plain
//! `#[test]`s so the bodies run on the main instance *outside* any drive — the
//! path a `#[tokio::test]` body (already inside a drive) cannot exercise.
...
use tokio::emscripten::event_loop::{drive, schedule};
...
fn schedule_does_not_run_root_synchronously() {
    ...
    assert!(drive(), "drive from host context must run a pick-up");
